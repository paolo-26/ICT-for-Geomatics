...
lambdaD = [dms2degrees([7 47 26.292]);
           dms2degrees([16 50 6.493])];  % Degrees

% Reference system: WGS84 - UTM 32 and UTM 33.
lambdaMc = [9; 15]; % Central meridian: 9° and 15°
...
% Variables.
lambda_1 = deg2rad(lambdaD - lambdaMc);
Rp = a^2 / c;
nu1 = sqrt(1+e_^2.*(cos(phi)).^2);
xi = atan(tan(phi)./cos(nu1.*lambda_1));
nu =  sqrt(1+e_^2*(cos(xi)).^2);
false_east = 5e5;

% Equations.
x = Rp * asinh(cos(xi).*tan(lambda_1)./nu);
y = a*(A1*xi - A2*sin(2*xi) + A4*sin(4*xi) - A6*sin(6*xi));

% Cartographic coordinates.
east = x*mc + false_east;
north = y*mc;
